...
def test_parse_factor():
    ...
    assert tokens[0]["tag"] == None # I've added this test to each test case to ensure the input has ended and nothing is leftover
    tokens = tokenize("!1")
    ast, tokens = parse_factor(tokens)
...
def parse_statement(tokens):
    """
    statement = statement_block| if_statement | while_statement |  function_statement | return_statement | print_statement | assignment_statement ;
    """
    tag = tokens[0]["tag"]
    if tag == "{":
